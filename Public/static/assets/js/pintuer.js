$(function () {
    ...
    $("form, .ys_record_table").on('blur', 'textarea, input, select', function(event) {
        var e = $(this);
    ...
    $pintuercheck = function (element, type, value) {
        ...
        case "radio":
            ...
            break;
        case "isAddress":
            return isAddress($pintu);
            break;
        case "litteMoney":
            return litteMoney($pintu);
            break;
        case "feeMoney":
            return feeMoney($pintu);
            break;
        default:
            var $test = type.split("#");
            if ($test.length > 1) {
    ...
    */
    ...
    var litteMoney = function (val) {
        if (parseInt(val) > parseInt($(".raising_money").text())) {
            return false;
        } else {
            return true;
        }
    };
    var feeMoney = function(val) {
        if (parseInt(val) < parseInt($(".raising_fee").text())) {
            return false;
        } else {
            return true;
        }
    ...
    */
    ...
    $.extend({
        isAddressETH: function(address) {
            return isAddress(address);
        }
    })
    // $.fn.isAddressETH = function(address) {
    //     return isAddress(address);
    // };
    ...
    $(".banner").each(function () {
        ...
        var itemHeight = Math.ceil(e.outerHeight() / items);
        var page = Math.ceil(num / items);
        //e.find(".carousel .item").css("width", itemWidth + "px");
        e.find(".carousel .item").css("height", itemHeight + "px");
        //e.find(".carousel").css("width", itemWidth * num + "px");
        e.find(".carousel").css("height", itemHeight * num + "px");
        ...
        if (pointer != 0 && page > 1) {
            ...
            e.append(pager);
            pager.css("top", e.outerHeight() * 0.5 - pager.outerHeight() * 0.5 + "px");
    ...
    $showbanner = function (e, i, items, num) {
        var after = 0,
            leftx = 0;
        leftx = -Math.ceil(e.outerHeight() / items) * (items) * (i - 1);
        if (i * items > num) {
            after = i * items - num;
            leftx = -Math.ceil(e.outerHeight() / items) * (num - items)
        }
